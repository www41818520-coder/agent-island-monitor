...
- **悬停展开**：鼠标放到迷你胶囊上会立即展开，移开后立即回到迷你胶囊。
- **快速过渡**：迷你胶囊和展开态之间使用短动画过渡，避免生硬跳变。
- **系统级拖动**：按住小岛拖动时交给 Windows 原生窗口拖动，适合避开浏览器或软件顶部控件。
...
- `Needs You`：状态变化时展开提醒；鼠标移走后仍回到迷你胶囊。
- 鼠标悬停迷你胶囊：立即展开状态胶囊。
- 鼠标离开小岛：立即回到迷你胶囊。
- 左键单击：不执行展开/收回功能。
- 左键按住拖动：交给 Windows 原生拖动，并在拖动结束后保存位置。
